...
def regenerate_image():
    ...
    try:
        ...
        # 记录日志
        logger.info(f"[重新生成] 任务 {task_id} 已添加到队列，文件: {filename}")
        ...
        while time.time() - start_time < timeout:
            ...
            # 超时处理
            logger.error(f"[重新生成] 任务 {task_id} 处理超时")
        ...
        if result.get('success'):
            logger.info(f"[重新生成] 任务 {task_id} 处理成功，文件: {filename}")
            ...
            logger.error(f"[重新生成] 任务 {task_id} 处理失败，文件: {filename}，错误信息: {result.get('message', '未知错误')}")
            error_msg = result.get('error', '生成失败')
            return jsonify({'success': False, 'message': error_msg}), 500
            
    except Exception as e:
        logger.error(f"[重新生成] 接口错误: {str(e)}")
